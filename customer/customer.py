...
class Customer(object):
    ...
    def get_field_of_view(self):
        ...
        field_of_view = self.compute_field_of_view(extra)

        return field_of_view

    def compute_field_of_view(self, extra):

        field_of_view = [self.x]

        if extra > 0:

            lower_bound = self.x - (extra // 2)

            if lower_bound <= 0:
                lower_bound = self.n_positions - lower_bound
                field_of_view += list(range(1, self.x))
                field_of_view += list(range(lower_bound, self.n_positions + 1))
            else:
                field_of_view += list(range(lower_bound, self.x))

            upper_bound = self.x + (extra // 2)
            if upper_bound > self.n_positions:
                upper_bound = upper_bound - self.n_positions
                field_of_view += list(range(self.x + 1, self.n_positions + 1))
                field_of_view += list(range(1, upper_bound + 1))
            else:
                field_of_view += list(range(self.x + 1, upper_bound + 1))
